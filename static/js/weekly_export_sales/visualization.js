    ...
    $('#visualization-form').submit(function(e) {
        ...
        if (commodityCode && startYear && endYear && metric && plotType) {
            ...
            $.ajax({
                    ...
                    if (response.success) {
                        ...
                        const summaryHtml = `
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <p><strong>Latest Week:</strong> ${mrp.formatNumber(summary.latest_week)} ${summary.units}</p>
                                    <p><strong>Last Updated:</strong> ${mrp.formatDate(summary.latest_date)}</p>
                                </div>
                            </div>
                        `;
                ...
                error: function() {
                    mrp.hideLoading('loading');
                    mrp.showError('plot-container', 'An error occurred while generating the plot. Please try again later.');
                }
            });
        } else {
