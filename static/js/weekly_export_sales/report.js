            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    if (report.report_date) {
                        $('#report-week').text(mrp.formatDate(report.report_date));
                    } else {
                        // Use current date as fallback
                        $('#report-week').text(mrp.formatDate(new Date()));
                    }
                    ...
                    // Show error message
                    mrp.showError('report-container', 'Error generating report: ' + response.error);
                }
            },
            error: function() {
                mrp.hideLoading('loading');
                mrp.showError('report-container', 'Server error when generating report');
            }
        });
    });
});